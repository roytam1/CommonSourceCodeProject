...
uint32_t BUBBLECASETTE::read_io8(uint32_t address)
{
	uint32_t val = 0xff;
	uint32_t offset;
	uint16_t mask;
	...
	} else if(bubble_type == BUBBLE_TYPE_64KB) {
		page_size_tmp = 0x20;
		mask = 0x7ff;
		media_size_tmp = 0x10000;
	...
	case 3: // Read Error register
		...
		break;
	case 4:
		val = page_address.b.h;
		break;
	case 5:
		val = page_address.b.l;
		break;
	case 6:
		val = page_count.b.h;
		break;
	...
	case 8:
		val = 0xff;
		if(bubble_inserted) {
			val &= ~0x80;	// page_address = 0000h
//			val &= ~0x40;	// page_address = 0400h
		}
		break;
	}
	return val;
...
void BUBBLECASETTE::write_io8(uint32_t address, uint32_t data)
{
	uint8_t val;
	uint32_t offset;
	uint16_t mask = 0;
	...
	case 0: // Write Data
		data_reg = val;
		if(stat_tdra) {
			if((offset = page_address.w.l * page_size_tmp + offset_reg) >= media_size_tmp) {
				povr_error = true;
				stat_error = true;
				cmd_error = true;
				return;
	...
	if(fio->IsFileExisting(file_path)) {
		...
		if(bubble_type != BUBBLE_TYPE_B77) {
			...
			case BUBBLE_TYPE_32KB:
				...
				break;
			case BUBBLE_TYPE_64KB:
				fio->Fread(bubble_data, 0x10000, 1);
				break;
			...
			do {
				...
				if(contain_medias != bank) {
					...
					if((uint32_t)fio->Ftell() >= file_length) return; // Error
				} else { // Image found
					if(bubble_type == BUBBLE_TYPE_32KB) {
...
//	uint32_t f_pos;
	...
	if ((uint32_t)fio->Ftell() >= file_length) return false;
...
//	uint32_t f_pos;
	uint8_t tmpval[16];
	if(fio == NULL) return;
	if((uint32_t)fio->Ftell() >= file_length) return;
	// You need convert to [UTF8|Local8Bit] when using UI.
	// Offset(little endian)
...
bool BUBBLECASETTE::read_one_page()
{
	...
	f_pos = media_offset;
	{
		...
		case BUBBLE_TYPE_32KB:
			...
			break;
		case BUBBLE_TYPE_64KB:
			offset = (page_address.w.l & 0x07ff) * 0x20;
			page_size = 0x20;
			break;
		case BUBBLE_TYPE_128KB:
