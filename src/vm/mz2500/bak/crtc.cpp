...
void CRTC::write_io8(uint32 addr, uint32 data)
{
	...
	case 0xf5:
		...
		if(0x80 <= textreg_num && textreg_num < 0x90) {
			...
			// update priority
			for(int i = 1; i < 8; i++)
				priority[c][i] = p ? c : (i + 16);
			priority[c][0] = c; // transparent black
			priority[c][8] = p ? c : (0 + 16); // non transparent black
...
{
	bool next = (GDEHS <= h && h < GDEHE) ? false : true;
	if(hblank != next) {
#ifdef VRAM_WAIT
...
void CRTC::draw_screen()
{
	...
	_memcpy(palette16txt, &palette16tmp[16], sizeof(uint16) << 3);
	palette16txt[0] = (back16 == 0 && palette_reg[0] == 2) ? 0 : palette16[palette_reg[back16]]; // tower of doruaga
	palette16txt[8] = 0;
	_memcpy(palette4096txt, &palette4096tmp[16], sizeof(uint16) << 3);
	...
	if(cgreg[0x0e] == 0x1d || cgreg[0x0e] == 0x9d) {
		...
		if(emu->get_capture_device_buffer()) {
			...
			for(int y = vs; y < ve && y < 400; y++) {
				...
				for(int x = hs; x < he && x < 640; x++) {
					uint16 col = palette4096tmp[priority[src_cg[x]][src_text[x]]];
					if(col) dest[x] = col;
				}
				for(int x = he; x < 640; x++) {
					uint16 col = palette16txt[src_text[x]];
					if(col) dest[x] = col;
				}
			}
			for(int y = ve; y < 400; y++) {
